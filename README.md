...
> **💡 VS Code users:** Images in this README are hosted online. To see them in VS Code preview, open the Command Palette (`Ctrl+Shift+P`) → search **"Markdown: Change Preview Security Settings"** → select **"Allow insecure content"**. On GitHub they render perfectly with no changes needed.
...
![bcryptjs](https://img.shields.io/badge/bcryptjs-orange?style=flat-square)
![Helmet](https://img.shields.io/badge/Helmet.js-grey?style=flat-square)
![Multer](https://img.shields.io/badge/Multer-green?style=flat-square)
...
│   │   ├── company.controller.js       # Register, update, fetch company
│   │   ├── job.controller.js           # Post, edit, search jobs
│   │   └── user.controller.js          # Register, login, profile update
│   ├── 🔒 middlewares/
...
└── 🖥️  frontend/
    └── src/
        ...
        │   │   ├── Applicants.jsx      # View all applicants
        │   │   ├── Companies.jsx       # Company management
        │   │   ├── PostJob.jsx         # Job posting form
        │   │   └── ProtectedRoute.jsx  # Role-guarded route
        │   ├── auth/
        │   │   ├── Login.jsx
        │   │   └── Signup.jsx
        │   ├── shared/
...
| `POST` | `/user/profile/update` | Update profile, photo, resume | ✅ |
 
<details>
<summary>📋 Register · Example</summary>
```http
...
| `GET` | `/company/get` | Get all companies by recruiter | ✅ |
| `GET` | `/company/get/:id` | Get company by ID | ✅ |
| `PUT` | `/company/update/:id` | Update company info | ✅ |
...
| `GET` | `/job/get` | Get all jobs (optional `?keyword=`) | ✅ |
| `GET` | `/job/get/:id` | Get job by ID | ✅ |
| `GET` | `/job/getadminjobs` | Get recruiter's own jobs | ✅ |
| `PUT` | `/job/update/:id` | Update job details | ✅ |
 
<details>
<summary>📋 Post Job · Example</summary>
```http
 ...
 
{
  ...
  "description": "Build responsive React UIs...",
...
| Method | Endpoint | Description | Auth |
|:------:|----------|-------------|:----:|
| `POST` | `/application/apply/:id` | Apply for a job | ✅ |
| `GET` | `/application/get` | Get user's applications | ✅ |
| `GET` | `/application/:id/applicants` | Get all applicants for a job | ✅ |
| `POST` | `/application/status/:id/update` | Update application status | ✅ |
...
```javascript
{
  ...
  profile: {
    ...
    profilePhoto:       String  (URL, default: "")
  },
  createdAt:   Date,
  updatedAt:   Date
}
...
├────────────────┼──────────────┼────────────────────────────────┤
│ JWT Token      │ Session      │ HTTP-only cookie (no JS access)│
│ Password       │ Permanent    │ bcryptjs hash in MongoDB       │
└────────────────┴──────────────┴────────────────────────────────┘
```
...
- [x] 🛡️ Helmet.js HTTP security headers
- [x] 🌐 CORS whitelist — frontend URL only
- [x] 📁 Multer file validation — size limits + type checks
- [x] 🔐 Protected API routes via `isAuthenticated` middleware
- [x] 🗃️ MongoDB unique index on email field
...
# 3. Commit your changes (conventional format)
...
Please follow the existing code style and test your changes before submitting a PR.
